...
def analyze_sentiment(data) -> dict:
    """각 응답을 긍정/중립/부정으로 분류하고 Python에서 실제 비율을 계산한다."""
    if not data:
    ...
    for item in data:
        ...
        if sentiment == "POSITIVE":
            ...
            raise ValueError(f"알 수 없는 감정 분류 결과입니다: {sentiment}")
...
아래 하나의 질문과 답변을 분석하여 POSITIVE, NEUTRAL, NEGATIVE 중 하나로 분류하세요.
질문과 답변의 의미를 함께 고려하고, SCORE 유형이라면 점수의 의미도 고려하세요.
...
"""
    response = ollama.chat(
        model=MODEL_NAME,
        messages=[
            {"role": "system", "content": "사용자 응답을 긍정, 중립, 부정으로 분류하고 반드시 JSON으로만 응답하세요."},
    ...
    if "sentiment" not in result:
        raise ValueError("AI 응답에 sentiment 값이 없습니다.")
    sentiment = str(result["sentiment"]).strip().upper()
    if sentiment not in {"POSITIVE", "NEUTRAL", "NEGATIVE"}:
        raise ValueError("감정 분석 결과는 POSITIVE, NEUTRAL, NEGATIVE 중 하나여야 합니다.")
    return sentiment
